...
% - The bolometric correction is maybe not defined in the AB magnitude system??
...
% - Do I clearly say that the SAME piece of glass in front of a CCD and a bolometer leads to different bandpasses? This should be in the transmission-function section.
...
\end{align}
where the function $A(\lambda)$ is very similar to the optical depth but with a factor of 1.086 that converts from natural-log units to magnitude units (because $2.5\logten(e)=1.086$ HOGG IS THIS ARGUMENT CORRECT?).
...
Such things have been proposed \cite{wacky}, and indeed the extremely precise \textsl{FIRAS} \cite{firas} measurements of the cosmic microwave background intensity---which are perhaps the most accurate astronomical measurements of intensity ever made---were made by making on-board comparisons to an on-board standard intensity source \cite{firascalibrator}.\footnote{In this sentence and the next I am intentionally using the word ``accurate''; I make a strong distinction between precision and accuracy.}

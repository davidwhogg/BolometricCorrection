...
% to-do items:
% ------------
% - Go through the lists above and below, and submit it to arXiv!
...
% - Note (and cite) that Vega is messed up so IN FACT the secondary standards define the Vega-relative system now.
% - Maybe mention and cite the Oke and Gunn work on calibrating Vega?? The blackbody source?? The fact that the calibration source drifted in temperature even though it couldn't possibly?
...
% Should I have a comment on the naming of UBVRIJHK?
...
Because the bolometric correction $BC$ is a ratio of (integrals of) luminosities of stars, and not a ratio of apparent brightnesses, the bolometric correction is a distance-independent (and, we will see below, interstellar-absorption-independent) property of the target star.
...
Indeed, the bolometric correction of the fundamental standard is not zero, even in the Vega-relative system, and thus it is not better than the Sun, conceptually, and the Sun is better understood, theoretically.
...
\begin{align}
    m_b &= m_\bol - BC_b ~,
\end{align}
but maybe these aren't used as much as the bolometric absolute magnitudes?
...
But when the bandpasses are wide, this is not true, fundamentally because the multiplication (absorption is multiplicative) and integration (the magnitude involves an integral) operators don't commute! This is discussed in detail in \cite{green}.
...
But it is hard to know precisely the individual reddening or extinction towards an individual star.
...
This would be the next generation for experiments like the one in which a platinum triple-point blackbody was placed on a tower near the Palomar 5-m telescope!\footnote{I can't find this experiment in the astronomical literature, but I was told about it many years ago by Jim Gunn (Princeton). His story involved some mysteries about the time-dependence of laboratory black-bodies, even those that are fixed to a fundamental triple point.}

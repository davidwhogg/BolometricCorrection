...
% - Audit for first person. We should use it sparingly, and be consistent about I vs we. I think I should do "I" now, not "we", but "we" still exists at places. Audit AGAIN.
...
the numerator being the signal from a detector (and filter) measuring the electromagnetic flux from the star of interest, and the denominator being the hypothetical (or actual) signal from that same detector (and filter) measuring a particular standard star (usually Vega, but sometimes another star like perhaps BD+17, or secondary standards, or hypothetical synthetic sources).
...
Although the Sun and Vega are about as well understood as stars can be, theorists are not always on the same page with regard to the Vega-relative Solar bolometric magnitude. It is important enough to astrophysical practice that the IAU adopted a reference value with resolution 2015.B2 to make the literature more definite. HOGG CITE}% HOGG CITE - IAU definition of a standard, IAU 2015.B3 (https://arxiv.org/abs/1510.06262)
...
But it's required, unfortunately, because most kinds of optical detectors are photon-counting devices, and yet most kinds of theory predict integrated energy outputs of stars, not integrated \emph{photon} outputs.

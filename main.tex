...
% to-do items:
% ------------
% - Blanton comment: Remove bolometry from sec 2, make a sec 3 about the meaning of the transmission function, discuss bolometer in there.
% - Is there a notation issue that we have $R(\lambda)$ but there are many different bandpasses? YES WE NEED TO SUBSCRIPT R(\lambda). Maybe check kcorrect paper for a position on notation.
...
% - Should we always subscript R() with (say) b? So there is always a defined "band" b?
...
The point of this definition \eqref{eq:mag} of the apparent magnitude is that it is very simple to very precisely measure the magnitude.
The definition is given in terms of integrals of the spectrum $f_\lambda(\lambda)$ of the source, and the spectrum $f^{(0)}_\lambda(\lambda)$ of the standard star, and the transmission function $R(\lambda)$ of the bandpass.
...
HOGG: Move everything about bolometers down here, and redefine $R(\lambda)$ to match Blanton's requirements.

HOGG: Explicitly compose $R(\lambda)$ as a product of functions for atmosphere, filter, optics, and detector.

HOGG: Look up names in the engineering systems (Gaia, STScI, 2MASS, etc) for these functions and audit this document to agree or be consistent.

HOGG: Note that the same piece of glass on a CCD and on a bolometer give different bandpasses.

HOGG: Note that if you measure a piece of glass in the standard way, you get its multiplicative contribution to $R(\lambda)$, if you are going to put that piece of glass in front of a photon detector.

HOGG: But also note that this is true in fine detail for every different pixel of every device!

\section{Color index}\label{sec:color}

Much of astronomy is the study of star (or galaxy or quasar or supernova or \etc) spectra or spectral energy distributions.\footnote{HOGG: This is jargon; explain it.}
...
For each of these bandpasses, if you want to interpret theoretically the spectral information it provides, it is critical to have some estimate of the transmission function $R(\lambda)$, as we discussed in \secref{sec:mag}.
...
% \section{Example: The \textsl{Gaia} bandpasses}\label{sec:gaia}

% HOGG ASKS: Do we need a section that does these calculations explicitly with bandpasses and flux densities? It could be useful, but it also seems a bit like a distraction. Thoughts, people?

% HOGG: Maybe show the $R(\lambda)$ functions for the three Gaia bands?

% HOGG: Maybe show a few spectral models (a few should be blackbodies, then maybe matched realistic stellar spectra)?
...
An absolute magnitude $M_R$ is related to an apparent magnitude $m_R$ through a \emph{distance modulus} $DM$, which is a logarithmic measure of the distance to the star:\footnote{Note that the distance modulus is traditionally denoted by the two-letter symbol ``$DM$'' and is not in any sense a product of a $D$ times an $M$.}
\begin{align}
    m_R &= M_R + DM\label{eq:absmag}\\
...
These are more theoretically interpretable than their absolute magnitudes $M_R$ in bandpass $R(\lambda)$.
Thus it was natural for astronomers to define a bolometric correction $BC_R$ which converts between the two:
\begin{align}
    M_R &= M_\bol - BC_R\label{eq:BC}\\
    m_R &= M_\bol - BC_R + DM ~,
\end{align}
...
Although the Sun and Vega are about as well understood as stars can be, theorists are not always on the same page with regard to the Vega-relative Solar bolometric magnitude. It is important enough to astrophysical practice that the IAU adopted a reference value with resolution 2015.B2 to make the literature more definite.}% CITE - IAU definition of a standard, IAU 2015.B3 (https://arxiv.org/abs/1510.06262)
